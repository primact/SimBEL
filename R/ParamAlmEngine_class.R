...
setValidity(
    ...
    function(object) {
        ...
        if (length(object@alloc_cible) != 4L) {
            retval <- c(
                retval,
                "[ParamAlmEngine] : L'attribut alloc_cible doit contenir quatre elements representant respectivement la proportion d'action, d'immobilisation, d'obligation et de tresorerie du portefeuille\n" # nolint: line_length_linter.
            )
        }
        if (sum(object@alloc_cible) != 1L | sum(object@alloc_cible > 1) > 0L | sum(object@alloc_cible < 0) > 0L) {
            retval <- c(
                retval,
                "[ParamAlmEngine] : Les elements de l'attribut alloc_cible doivent etre compris entre 0 et 1 (hors tresorerie) et leur somme doit etre egale a 1.\n" # nolint: line_length_linter.
            )
        }
        if (length(object@seuil_realisation_PVL) != 1L) {
            retval <- c(
                retval,
                "[ParamAlmEngine] : Le seuil de realisation de PVL ne peut contenir plus d'un element\n"
            )
        }
