...
setMethod(
  ...
  definition = function(x, mp_ESG){
    ...
    index_action <- .subset2(x@ptf_action@ptf_action, num_index_action)
    index_immo <- .subset2(x@ptf_immo@ptf_immo, num_index_immo)
    ...
    # Gestion des noms de colonnes du data.frame de donnnees
    nom_table_mp_immo <- names(mp_ESG@indice_immo)
    ncol_S_immo <- which(nom_table_mp_immo == "S_immo")
    ncol_S_prev_immo <- which(nom_table_mp_immo == "S_prev_immo")

    # Rendement immo
    S_immo <- .subset2(mp_ESG@indice_immo, ncol_S_immo)[index_immo]
    S_prev_immo <- .subset2(mp_ESG@indice_immo, ncol_S_prev_immo)[index_immo]
