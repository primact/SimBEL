...
##' @note Cette methode cree l'architecture, puis les objets \code{\link{Be}} correspondant a chacun des scenarios :
##' central et de chocs de la formule standard.
##' @author Prim'Act
##' @export
##' @include Initialisation_class.R


setGeneric(name = "init_scenario", def = function(x) {
    standardGeneric("init_scenario")
})
setMethod(
    f = "init_scenario",
        ...
        for (name_scenario in table_choc@scenario) {
            ...
            if (name_scenario != "taux_up" & name_scenario != "taux_down") {
                ...
                if (name_scenario == "frais") {
                    table_ESG <- get_choc_inflation_frais(
                        table_ESG,
                ...
                input_autres_passifs_choc <- read.csv2(
                    paste(x@address[["data"]][["autres_passifs_choc"]], "noms_liens_autres_passifs_choc.csv", sep = "/"),
