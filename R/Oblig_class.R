...
setValidity(
    ...
    function(object) {
        ...
        if (dim(ptf_oblig)[2L] != nb_col) {
            retval <- c(
                retval,
                "[Oblig] : Nombre d'attributs incorrect, un ptf Oblig est composE d'un DF de 12 colonnes /n"
            )
        }
        ...
        if (sum(colnames(ptf_oblig) == c(
            "num_mp", "val_marche", "val_nc", "val_achat",
...
setMethod(
    ...
    definition = function(.Object, ptf = data.frame()) {
        ...
        if (!missing(ptf)) {
            ...
            if (ncol(ptf) != nb_col | sum(names(ptf) != c(
                "num_mp", "val_marche", "val_nc", "val_achat",
                "presence", "cessible", "nb_unit", "dur_det",
                "nominal", "tx_coupon", "par", "mat_res", "type",
                "rating", "duration", "zspread", "cc", "sd",
                ...
                !is.integer(.subset2(ptf, num_mp)) | !is.numeric(.subset2(ptf, val_marche)) | !is.numeric(.subset2(ptf, val_nc)) |
                    !is.numeric(.subset2(ptf, val_achat)) | !is.logical(.subset2(ptf, presence)) | !is.logical(.subset2(ptf, cessible)) |
                    !is.numeric(.subset2(ptf, nb_unit)) | !is.numeric(.subset2(ptf, dur_det)) | !is.numeric(.subset2(ptf, nominal)) |
                    !is.numeric(.subset2(ptf, tx_coupon)) | !is.numeric(.subset2(ptf, par)) | !is.numeric(.subset2(ptf, mat_res)) |
                    !is.factor(.subset2(ptf, type)) | !is.integer(.subset2(ptf, rating)) | !is.numeric(.subset2(ptf, duration)) |
                    !is.numeric(.subset2(ptf, zspread)) | !is.numeric(.subset2(ptf, cc)) | !is.numeric(.subset2(ptf, sd)) |
            ...
            colnames(.Object@ptf_oblig) <- c(
                "num_mp", "val_marche", "val_nc", "val_achat", "presence", "cessible", "nb_unit",
                "dur_det", "nominal", "tx_coupon", "par", "mat_res", "type", "rating", "duration",
                "zspread", "cc", "sd", "character", "numeric"
            )
        }
        return(.Object)
    }
