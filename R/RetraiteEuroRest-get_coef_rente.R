...
##'
setGeneric("get_coef_rente", function(x, ht) {
    standardGeneric("get_coef_rente")
})
setMethod(
    f = "get_coef_rente",
    ...
    def = function(x, ht) {
        ...
        calc_coef_rente_mp <- function(i) {
            ...
            if (statut_i == 1L) { # Cas avec une seule tete
                return(calc_ax(
                    tx_tech_i, freq_i, echu_i, tables_mort[[tab_mort_i]], age_i, gen_i
                ))
            } else if (statut_i == 2L) { # Cas avec deux tetes
                return(calc_axy(
                    tx_tech_i, tx_rvs_i, freq_i, echu_i, tables_mort[[tab_mort_i]], age_i, gen_i, tables_mort[[tab_mort_rvs_i]], age_rvs_i, gen_rvs_i
                ))
            } else if (statut_i == 3L) { # Cas du reversataire seul
                return(tx_rvs_i * calc_ax(
                    tx_tech_i, freq_i, echu_i, tables_mort[[tab_mort_rvs_i]], age_rvs_i, gen_rvs_i
                ))
            } else {
