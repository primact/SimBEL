...
setGeneric(name = "finance_cible_marge", def = function(marge_fin, bes_cible, rev_stock_nette, marge_min){standardGeneric("finance_cible_marge")})
setMethod(
    ...
    definition = function(marge_fin, bes_cible, rev_stock_nette, marge_min){
        ...
        # Controle
        if(prod(bes_cible < 0)) stop("[RevaloEngine-finance_cible_marge] : les besoins de financements par produit doivent etre positifs.")
        if(length(bes_cible) != length(rev_stock_nette)) stop("[RevaloEngine-finance_cible_marge] : les vecteurs en entree ne sont pas de meme longueur.")
        ...
        if(bes_add < 0) {
            ...
            if(add_rev_tx_cib > 0)
                rev_stock_nette_cible[prod_avantage] <- bes_cible[prod_avantage] + add_rev_tx_cib *  bes_add_ind[prod_avantage] / sum(bes_add_ind[prod_avantage])
