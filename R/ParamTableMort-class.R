...
setClass(
    ...
    validity = function(object) {
        ...
        if (dim(object@table)[2] != nb_col_attentu) {
            retval <- c(retval, "[ParamTableMort] : Nombre d'attributs incorrect /n")
        }

        # Verification du type des colonnes
        if (!is.integer(object@table[, 1])) {
            retval <- c(retval, "[ParamTableMort] : gen n'est pas entier/n")
        }
        if (!is.integer(object@table[, 2])) {
            retval <- c(retval, "[ParamTableMort] : age n'est pas entier/n")
        }
        if (!is.numeric(object@table[, lx_num])) {
            retval <- c(retval, "[ParamTableMort] : lx n'est pas un reel /n")
        }
        if (!is.numeric(object@table[, qx_num])) {
            retval <- c(retval, "[ParamTableMort] : qx n'est pas un reel /n")
        }

        if (sum(object@table[, 1] <= 0) != 0) {
            retval <- c(retval, "[ParamTableMort] : gen <= 0 /n")
        }
        if (sum(object@table[, 2] < 0) != 0) {
            retval <- c(retval, "[ParamTableMort] : age < 0 /n")
        }
        if (sum(object@table[, lx_num] < 0) != 0) {
            retval <- c(retval, "[ParamTableMort] : lx < 0 /n")
        }
        if (sum(object@table[, qx_num] < 0) != 0) {
            retval <- c(retval, "[ParamTableMort] : qx < 0 /n")
        }
