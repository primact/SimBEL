...
#           update_dur_oblig
#----------------------------------------------------------------------------------------------------------------------------------------------------
##' Mise a jour des durations d'un portefeuille obligataire.
##'
...
##' @param duration un vecteur de \code{numeric} a assigner a l'objet \code{\link{Oblig}}.
##' @return L'objet \code{x} dont les durations ont ete mises a jour.
##' @author Prim'Act
##' @export
##' @include Oblig_class.R

setGeneric(name = "update_dur_oblig", def = function(x, duration) {
    standardGeneric("update_dur_oblig")
})
setMethod(
    f = "update_dur_oblig",
    ...
    definition = function(x, duration) {
        ...
        if (sum(duration < 0) > 0) {
            stop("[Oblig : update_dur_oblig] :  Le vecteur de duration initialement entre ne peut contenir de valeurs negatives. \n")
        }
