...
setGeneric("calc_ax", function(tx_tech, freq_rente, echu,
                                       table_mort, age, gen){
  standardGeneric("calc_ax")})
setMethod(
  f = "calc_ax",
  signature = c(tx_tech = "numeric", freq_rente = "integer", echu = "logical",
  ...
  def = function(tx_tech, freq_rente, echu, table_mort, age, gen){
    ...
    nPeriodes = max(table_mort@age_max - age, as.integer(1))

    # Calcul de l'ax echu
    ax <- sum(calc_proba_survie(table_mort, age, gen, nPeriodes) *
                (1 + tx_tech)^-(1:(nPeriodes))) + (freq_rente - 1)/(2 * freq_rente)
...
##' @include ParamTableMort-class.R
##'
setGeneric("calc_axy", function(tx_tech, tx_rvs, freq_rente, echu,
                                       table_mort_1, age_1, gen_1,
                                       table_mort_2, age_2, gen_2){
  standardGeneric("calc_axy")})
setMethod(
    f = "calc_axy",
    signature = c(tx_tech = "numeric", tx_rvs = "numeric", freq_rente = "integer", echu = "logical",
                  table_mort_1 = "ParamTableMort", age_1 = "integer", gen_1 = "integer",
                  table_mort_2 = "ParamTableMort", age_2 = "integer", gen_2 = "integer"),
    def = function(tx_tech, tx_rvs, freq_rente, echu,
                   table_mort_1, age_1, gen_1,
                   table_mort_2, age_2, gen_2){
      # Bornes de la sommation
      nPeriodes1 =  max(table_mort_1@age_max - age_1, as.integer(1)) # Premiere tete
      nPeriodes2 =  max(table_mort_2@age_max - age_2, as.integer(1)) # Deuxieme tete
