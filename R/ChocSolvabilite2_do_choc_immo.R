...
##' de l'assureur.
##' @return \code{canton} l'objet  de la classe \code{\link{Canton}} correspondant au scenario choque immobilier au sens
##' de la formule standard Solvabilite 2.
...
setGeneric(name = "do_choc_immo", def = function(x, canton) {
    standardGeneric("do_choc_immo")
})
setMethod(
    f = "do_choc_immo",
    ...
    definition = function(x, canton) {
        ...
        if (nrow(canton@ptf_fin@ptf_immo@ptf_immo) == 0) {
            stop("[choc Mket : Immo] : tentative de calcul du choc immo avec un objet Immo vide impossible. \n")
        }
        ...
        split_ptf_immo <- lapply(1:length(index), function(x) {
            split_ptf_immo[[x]] <- temp[which(temp$num_index == index[x]), ]
        })

        # Appliquer le choc par bloc de numero d'index
        split_ptf_immo <- lapply(1:length(index), function(x) {
            split_ptf_immo[[x]]$val_marche <- (1 - table_choc_immo[which(table_choc_immo$num_index == index[x]), "choc_immo"]) *
                split_ptf_immo[[x]]$val_marche
        ...
        if (nrow(canton@param_alm@ptf_reference@ptf_immo@ptf_immo) == 0) {
            stop("[choc Mket : Immo] : Portefeuille de reference Immo vide - application du choc impossible. \n")
        }
        temp <- canton@param_alm@ptf_reference@ptf_immo@ptf_immo
        # Methode bourrine : le portefeuille de reference est cense etre de petite taille --> il ne necessite pas les etapes de
        # decoupages effectues pour le portefeuille immo standard qui peut devenir gros
        ...
        vecteur_choc_immo_ref <- unlist(lapply(1:length(index), function(x) {
            vecteur_choc_immo_ref <- c(vecteur_choc_immo_ref, table_choc_immo[which(table_choc_immo$num_index == index[x]), "choc_immo"])
        }))
        # Application du choc sur le portefeuille de reference
        canton@param_alm@ptf_reference@ptf_immo@ptf_immo$val_marche <- (1 - vecteur_choc_immo_ref) *
            canton@param_alm@ptf_reference@ptf_immo@ptf_immo$val_marche
