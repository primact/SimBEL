...
##' @name load_action
##' @docType methods
##' @param file_action_address est un \code{character} contenant l'adresse exacte du fichier d'input utilisateur renseignant le portefeuille.
...
setGeneric(name = "load_action", def = function(file_action_address) {
    standardGeneric("load_action")
})
setMethod(
    f = "load_action",
    signature = "character",
    definition = function(file_action_address) {
        # Lecture du fichier
        temp <- read.csv2(file_action_address, colClasses = c(
