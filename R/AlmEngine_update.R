...
setGeneric(name = "update_journal", def = function(x,operation_achat_vente){standardGeneric("update_journal")})
setMethod(
  f = "update_journal",
  signature = c(x = "AlmEngine", operation_achat_vente = data.frame()),
  definition = function(x, operation_achat_vente = data.frame(annee      = integer(),
                                                            operation  = "",
                                                            type_actif = "",
                                                            num_mp     = integer(),
                                                            num_index  = integer(),
  ...
  definition = function(x, ptf, annee, operation){
    ...
    if (operation == "achat") {
      ...
      operation_achat_vente = data.frame(annee      = rep(annee, nrow(ptf["ptf_action"])),
                                         ...
                                         nb_unit    = ptf["ptf_action"][,"nb_unit"],
                                         pmvr       = pmvr)
    } else {stop("[AlmEngine : update_journal_action] : L'operation realisee est soit un achat soit une vente \n")}
...
setGeneric(name = "update_journal_immo", def = function(x, ptf, annee, operation, pmvr){standardGeneric("update_journal_immo")})
setMethod(
  ...
  definition = function(x, ptf, annee, operation, pmvr){
    ...
    if (operation == "achat") {
      operation_achat_vente = data.frame(annee      = rep(annee, nrow(ptf["ptf_immo"])),
      ...
      operation_achat_vente = data.frame(annee      = rep(annee, nrow(ptf["ptf_immo"])),
                                         ...
                                         nb_unit    = ptf["ptf_immo"][,"nb_unit"],
                                         pmvr       = pmvr)
    } else {stop("[AlmEngine : update_journal_immo] : L'operation realisee est soit un achat soit une vente \n")}
