...
setValidity(
    ...
    function(object) {
        ...
        if (sum(colnames(object@ptf_action) == c(
            "num_mp", "val_marche", "val_nc", "val_achat",
            "presence", "cessible", "nb_unit", "dur_det",
            "pdd", "num_index", "div", "ind_invest", "currency", "fx_rate"
        )) != 14L) {
        ...
        if (!missing(ptf)) {
            ...
            if (ncol(ptf) != 14L | sum(names(ptf) != c(
                ...
                !is.integer(.subset2(ptf, num_mp)) | !is.numeric(.subset2(ptf, val_marche)) | !is.numeric(.subset2(ptf, val_nc)) |
                    ...
                    !is.integer(.subset2(ptf, num_index)) | !is.numeric(.subset2(ptf, div)) | !is.logical(.subset2(ptf, ind_invest)) |
