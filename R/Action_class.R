...
setValidity ("Action",
             function (object){
               ...
               if (!is.integer(.subset2(object@ptf_action, 1)))   {retval <- c(retval, "[Action] : num_mp n'est pas entier/n")}
               if (!is.numeric(.subset2(object@ptf_action, 2)))   {retval <- c(retval, "[Action] : val_marche n'est pas reel/n")}
               if (!is.numeric(.subset2(object@ptf_action, 3)))   {retval <- c(retval, "[Action] : val_nc n'est pas reel/n")}
               if (!is.numeric(.subset2(object@ptf_action, 4)))   {retval <- c(retval, "[Action] : val_achat n'est pas reel/n")}
               if (!is.logical(.subset2(object@ptf_action, 5)))   {retval <- c(retval, "[Action] : presence n'est pas logical/n")}
               if (!is.logical(.subset2(object@ptf_action, 6)))   {retval <- c(retval, "[Action] : cessible n'est pas logical/n")}
               if (!is.numeric(.subset2(object@ptf_action, 7)))   {retval <- c(retval, "[Action] : nb_unit n'est pas reel/n")}
               if (!is.numeric(.subset2(object@ptf_action, 8)))   {retval <- c(retval, "[Action] : dur_det n'est pas reel/n")}
               if (!is.numeric(.subset2(object@ptf_action, 9)))   {retval <- c(retval, "[Action] : pdd n'est pas reel/n")}
               if (!is.integer(.subset2(object@ptf_action, 10)))  {retval <- c(retval, "[Action] : num_index n'est pas integer/n")}
               if (!is.numeric(.subset2(object@ptf_action, 11)))  {retval <- c(retval, "[Action] : div n'est pas reel/n")}
               if (!is.logical(.subset2(object@ptf_action, 12)))  {retval <- c(retval, "[Action] : ind_invest n'est pas logical/n")}
