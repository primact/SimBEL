...
setMethod(
    ...
    definition = function(x, num_sold, nb_sold){
        ...
        if(sum(num_sold %in% .subset2(x@ptf_action, num_mp))!= length(num_sold)) {stop("[Action : sell] : Tentative de vente d'une action non existante \n")}

        pmvr_part <- 0
        pmvr_tot  <- 0

        # Selection des lignes soumises a une operation de vente
        temp <- x@ptf_action[which(.subset2(x@ptf_action, num_mp) %in% num_sold),]

        # Verification qu'on ne vende pas plus d'unite qu'on n'en possede ou qu on ne vende pas une ligne non cessible
        if(sum(nb_sold > .subset2(temp, nb_unit)) > 0)   {stop("[Action : sell] : Vente d'action entraine une position negative \n")}
        if(sum(.subset2(temp,cessible) == F) > 0)       {stop("[Action : sell] : Tentative de vente d'une action non cessible \n")}
        if(sum(.subset2(temp,presence) == F) > 0)       {stop("[Action : sell] : Tentative de vente d'une action non presente \n")}

        # Premier cas : vente partielle d'une ligne action
        temp_part               <- new("Action", temp[which(.subset2(temp, nb_unit) > nb_sold),])
        num_sold_part           <- .subset2(temp_part@ptf_action, num_mp)
        nb_sold_part            <- nb_sold[which(num_sold %in% num_sold_part)]
        # Operations
        if(length(num_sold_part) > 0){
            pmvr_part <-   sum((.subset2(temp_part@ptf_action, val_marche) - .subset2(temp_part@ptf_action, val_nc))  * nb_sold_part / .subset2(temp_part@ptf_action, nb_unit))
            temp_part@ptf_action$val_achat  <- .subset2(temp_part@ptf_action, val_achat)  * (1 - nb_sold_part / .subset2(temp_part@ptf_action, nb_unit))
            temp_part@ptf_action$val_marche <- .subset2(temp_part@ptf_action, val_marche) * (1 - nb_sold_part / .subset2(temp_part@ptf_action, nb_unit))
            temp_part@ptf_action$val_nc     <- .subset2(temp_part@ptf_action, val_nc)     * (1 - nb_sold_part / .subset2(temp_part@ptf_action, nb_unit))
            temp_part@ptf_action$nb_unit    <- .subset2(temp_part@ptf_action, nb_unit) - nb_sold_part
        }

        # Deuxieme cas : vente totale d'une ligne action
        temp_tot     <- new("Action", temp[which(.subset2(temp, nb_unit) == nb_sold),])
        num_sold_tot <- .subset2(temp_tot@ptf_action, num_mp)
        if (length(num_sold_tot) > 0) {
          pmvr_tot <- sum(.subset2(temp_tot@ptf_action, val_marche) - .subset2(temp_tot@ptf_action,val_nc))
          }

        # Mise a jour de l'objet action
        # 1 : Suppression des lignes action integralement vendues
        if (length(num_sold_tot) > 0) {
          x@ptf_action <- x@ptf_action[-which(.subset2(x@ptf_action, num_mp) %in% num_sold_tot), ]}
        # 2 : Ajustement des lignes actions partiellement vendues
        if(length(num_sold_part) > 0) {
          x@ptf_action[which(.subset2(x@ptf_action, num_mp) %in% num_sold_part),] <- temp_part@ptf_action}
        # 3 : Message de prevention si le portefeuille est vide
        if(nrow(x@ptf_action) == 0) {warning(" [Action : sell] : Attention : portefeuille action vide")}

        # 4 : vider les actions dont la valeur nette comptable est inferieure a 0.001
        x@ptf_action <- x@ptf_action[which(.subset2(x@ptf_action, val_nc) > SEUIL_DEL_ACTION),]
