...
##' @docType methods
##' @param x objet de la classe \code{PortFin}, correspondant au portefeuille financier
...
##' @param flux_fin est un \code{numeric} correspondant aux revenus percus en fin d'annee
##'  (tombees d'echeances et revenus de tresorerie).
##' @param table_rdt est une \code{liste}, construite par la fonction \code{\link{calc_rdt}}.
##' Cette table contient les tables d'evolution des cours et rendements sur l'annee consideree de chacune
##'  des classes d'actif. Les tables sont constuites a partir des extractions du Generateur de Scenario Economique de Prim'Act.
##' @return L'objet renvoye de la classe \code{PortFin} correspond au portefeuille financier initial dont l'attribut \code{ptf_treso}
##' a ete vieilli d'une annee.
##' @author Prim'Act
##' @export
##' @seealso La fonction de calcul des rendements des actifs \code{\link{calc_rdt}}.
##' @include PortFin_class.R

setGeneric(name = "vieillissement_treso_PortFin", def = function(x, flux_milieu, flux_fin, table_rdt) {
    standardGeneric("vieillissement_treso_PortFin")
})
setMethod(
    ...
    definition = function(x, flux_milieu, flux_fin, table_rdt) {
        ...
        if (nrow(ptf_treso@ptf_treso) > 0) {
            ...
            # Mise a jour de la treso en fin d'annee (revalo de la treso  en tenant compte des flux de loyer/div/coupons de milieu d'annee
            # + prise en comtpe des flux de tombee d'echeance percus en fin d'annee)
