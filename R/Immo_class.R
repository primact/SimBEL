...
setValidity(
    ...
    function(object) {
        ...
        if (dim(ptf_immo)[2L] != nb_col_attendu) {
            retval <- c(
                retval,
                "[Immo] : Nombre d'attributs incorrect, un ptf Immo est compose d'un DF de 14 colonnes /n"
            )
        }
        ...
        if (sum(colnames(ptf_immo) == c(
            "num_mp", "val_marche", "val_nc", "val_achat", "presence", "cessible", "nb_unit", "dur_det",
...
setMethod(
    ...
    definition = function(.Object, ptf = data.frame()) {
        ...
        if (!missing(ptf)) {
            ...
            if (ncol(ptf) != 14L | sum(names(ptf) != c(
                "num_mp", "val_marche", "val_nc", "val_achat", "presence", "cessible", "nb_unit", "dur_det",
                "pdd", "num_index", "loyer", "ind_invest",
                "currency", "fx_rate"
            )) != 0L) {
                ...
                !is.integer(.subset2(ptf, num_mp)) | !is.numeric(.subset2(ptf, val_marche)) | !is.numeric(.subset2(ptf, val_nc)) |
                    ...
                    !is.numeric(.subset2(ptf, nb_unit)) | !is.numeric(.subset2(ptf, dur_det)) | !is.numeric(.subset2(ptf, pdd)) |
                    !is.integer(.subset2(ptf, num_index)) | !is.numeric(.subset2(ptf, loyer)) | !is.logical(.subset2(ptf, ind_invest)) |
            ...
            .Object@ptf_immo <- data.frame(
                integer(), numeric(), numeric(), numeric(), logical(),
                logical(), numeric(), numeric(), numeric(), integer(), numeric(), logical(),
                character(), numeric()
            )
            colnames(.Object@ptf_immo) <- c(
                "num_mp", "val_marche", "val_nc", "val_achat", "presence",
