...
setValidity ("Immo",
             function (object){
                 ...
                 if(dim(ptf_immo)[2L] != nb_col_attendu) retval <- c(retval, "[Immo] : Nombre d'attributs incorrect, un ptf Immo est compose d'un DF de 12 colonnes /n")
                 ...
                 if(sum(colnames(ptf_immo)==c("num_mp","val_marche","val_nc","val_achat", "presence","cessible","nb_unit","dur_det",
                                              "pdd","num_index","loyer","ind_invest")) != nb_col_attendu)
...
setMethod(
    ...
    definition = function(.Object, ptf=data.frame()){
        ...
        if( ! missing(ptf)){
            ...
            if(ncol(ptf) != 12L | sum(names(ptf)!=c("num_mp","val_marche","val_nc","val_achat", "presence","cessible","nb_unit","dur_det",
                                                    "pdd","num_index","loyer","ind_invest")) != 0L){
