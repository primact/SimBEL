...
setGeneric(name = "finance_cible_pmvl", def = function(bes_cible, rev_stock_nette, base_fin, seuil_pmvl, tx_pb){standardGeneric("finance_cible_pmvl")})
setMethod(
    ...
    definition = function(bes_cible, rev_stock_nette, base_fin, seuil_pmvl, tx_pb){
        ...
        # Controle
        if(prod(bes_cible < 0)) stop("[RevaloEngine-finance_cible_pmvl] : les besoins de financements par produit doivent etre positifs.")
        if(prod(tx_pb < 0)) stop("[RevaloEngine-finance_cible_pmvl] : les taux de PB doivent etre positifs.")
        ...
        if(bes_add < 0){
            ...
            if(sum(bes_add_ind) !=0) {
                ...
                if(sum(base_fin) !=0)
                    tx_pb_moy <- sum(base_fin * tx_pb / sum(base_fin)) # Calcul du taux de PB moyen
                else
                ...
                if(pmvl_liq == 0){ # Gestion des division par 0
                    suppl_revalo <-0 # Supplement de revalorisation
                } else {
