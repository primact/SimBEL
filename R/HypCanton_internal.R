...
setMethod(
  ...
  definition = function(x, i){
    switch(EXPR = i,
           "tx_soc" = {return(x@tx_soc)},
           "tx_import" = {return(x@tx_import)},
           "method_taux_cible" = {return(x@method_taux_cible)},
...
setReplaceMethod(
  ...
  definition = function(x, i, value){
    switch(EXPR = i,
           "tx_soc" = {x@tx_soc <- value},
           "tx_import" = {x@tx_import <- value},
           "method_taux_cible" = {x@method_taux_cible <- value},
           stop("[HypCanton] : Cet attribut n'existe pas!")
    )
    validObject(x)
    return(x)
