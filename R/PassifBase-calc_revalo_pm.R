...
setMethod(
    ...
    def = function(x, y){
        # Verification inputs
        if (length(y) != 2)                            stop("[EpEuroInd : calc_revalo_pm] : L'input y doit correspondre a une liste de longueur 2. \n")
        # Verification des noms des elements de la liste
        if (sum(names(y) == c("rev_net_alloue", "tx_soc")) != length(y)) stop("[EpEuroInd : calc_revalo_pm] : L'input y doit correspondre a une liste de longueur 2 de nom : rev_net_alloue, tx_soc . \n")
        ...
        if (! is.numeric(rev_net_alloue)) stop("[EpEuroInd : calc_revalo_pm] : L'input y doit correspondre a une liste de longueur 2, de nom : rev_net_alloue, tx_soc, dont le type est : numeric, numeric. \n")
        if (! is.numeric(tx_soc))         stop("[EpEuroInd : calc_revalo_pm] : L'input y doit correspondre a une liste de longueur 2, de nom : rev_net_alloue, tx_soc, dont le type est : numeric, numeric. \n")
...
setMethod(
    ...
    def = function(x, y){
        ...
        # Verification des types des elements de la liste
        if (! is.numeric(y[["rev_net_alloue"]])) stop("[RetraiteEuroRest : calc_revalo_pm] : L'input y doit correspondre a une liste de longueur 1, de nom : rev_net_alloue, dont le type est : numeric.")
