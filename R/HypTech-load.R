...
##'
setGeneric(name = "load_ht", def = function(x) {
    standardGeneric("load_ht")
})
setMethod(
    f = "load_ht",
    ...
    def = function(x) {
        ...
        input_morta <- read.csv2(
            paste(address[["param"]][["tables"]], "empl_tables_morta.csv", sep = "/"),
            header = TRUE, colClasses = c("character", "character", "character")
        )
        input_rach <- read.csv2(
            paste(address[["param"]][["tables"]], "empl_tables_rachats.csv", sep = "/"),
            header = TRUE, colClasses = c("character", "character", "character")
        )
        input_param_rach <- read.csv2(
            paste(address[["param"]][["tables"]], "empl_param_rachats_conj.csv", sep = "/"),
        ...
        for (i in 1:nrow(input_morta["nom_table_R"])) {
            # Lecture de la table
            temp_csv <- read.csv2(
                paste(address[["param"]][["tables"]], input_morta[i, "nom_table_csv"], sep = "/"),
        ...
        for (i in 1:nrow(input_rach["nom_table_R"])) {
            # Lecture du fichier
            temp_csv <- read.csv2(
                paste(address[["param"]][["tables"]], input_rach[i, "nom_table_csv"], sep = "/"),
                header = TRUE, colClasses = c("integer", "integer", "numeric")
            )
        ...
        for (i in 1L:nrow(input_param_rach["nom_table_R"])) {
            # Lecture du fichier
            temp_csv <- read.csv2(paste(address[["param"]][["tables"]], input_param_rach[i, "nom_table_csv"], sep = "/"),
                header = TRUE,
        ...
        param_comport_csv <- read.csv2(paste(address[["param"]][["tables"]], "param_comport.csv", sep = "/"),
            header = TRUE,
        ...
        alloc_mar <- as.numeric((c(
            param_comport_csv["alloc_mar_action"],
