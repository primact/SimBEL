# 06/03/2017 Guillaume de Kervenoael

#----------------------------------------------------------------------------------------------------------------------------------------------------
#           Constructeur general
#----------------------------------------------------------------------------------------------------------------------------------------------------
##' Cette methode construit l'objet de classe \code{ESG} a partir des donnees .csv issues de l'ESG Prim'Act.
##'
##' \code{chargement_ESG} est une methode permettant de construire et charger les trajectoires simulees par le Generateur de
##' Scenarios Economiques de Prim'Act.
##' @name chargement_ESG
##' @docType methods
##' @param folder_ESG_address est un \code{character}. Il correspond au chemin de reference du dossier contenant les extractions de l'ESG Prim'Act.
##' Les differentes adresses potentielles pour les differents ESG employes (central, hausse de taux, baisse de taux, avec ou sans VA)
##' sont construites par la fonction \code{\link{set_architecture}} de la classe \code{\link{Initialisation}}.
##' @param nb_simu est une valeur de type \code{integer} correspondant au nombre de trajectoire simulees par l'ESG Prim'Act.
##' @param nb_annee_proj est une valeur de type \code{integer} correspondant au nombre d'annees de projection des sorties de l'ESG Prim'Act.
...
setMethod(
    ...
    definition = function(folder_ESG_address, nb_simu, nb_annee_proj){
        ...
        yield_curve <- lapply(0:nb_annee_proj,function(y){
            ...
            if(ncol(temp_curve) != 30 ) {
                warning(paste("[ESG chargement : yield_curve] : Les courbes de taux spot forward ", y," ans, ne contiennent pas exactement 30 maturites."))
            } else {
                if(colnames(temp_curve) != paste("X", 1:30, sep = "")) warning(paste("[ESG chargement : yield_curve] : Les colonnes des courbes de taux spot forward ", y, "ans, ne sont pas renseignees selon la convention retenue pour les maturites de 1 a 30 (le nommage des colonnes doit etre 1,2,..., 30).", sep = ""))
            }
