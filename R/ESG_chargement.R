...
setGeneric(name = "chargement_ESG", function(folder_ESG_address, nb_simu, nb_annee_proj) {
    standardGeneric("chargement_ESG")
})
setMethod(
    f = "chargement_ESG",
    ...
    definition = function(folder_ESG_address, nb_simu, nb_annee_proj) {
        ...
        file_name <- read.csv2(
            paste(folder_ESG_address, "noms_liens.csv", sep = "/"),
            header = TRUE, colClasses = c(rep("character", 3), "integer")
        )[1:6, 1:4]
        ...
        if (length(id_indices_action$num_index) != length(unique(id_indices_action$num_index)) |
            length(id_indices_action$num_index) != nrow(id_indices_action)) {
            stop("[ESG chargement indices actions] : Veuillez indiquer un numero d'index unique par index action. \n")
        }
        if (!is.integer(id_indices_action$num_index) | sum(sort(id_indices_action$num_index) != c(1:length(id_indices_action$num_index))) > 0) {
            stop("[ESG chargement] : Les numeros d'index action doivent etre des entiers, l'indicage doit demarrer a 1 et etre continu par pas de 1.")
        }
        if (length(id_indices_immo$num_index) != length(unique(id_indices_immo$num_index)) |
            length(id_indices_immo$num_index) != nrow(id_indices_immo)) {
            stop("[ESG chargement indices immo] : Veuillez indiquer un numero d'index unique par index immo. \n")
        }
        if (!is.integer(id_indices_immo$num_index) | sum(sort(id_indices_immo$num_index) != c(1:length(id_indices_immo$num_index))) > 0) {
            stop("[ESG chargement] : Les numeros d'index immo doivent etre des entiers, l'indicage doit demarrer a 1 et etre continu par pas de 1.")
        }

        # Creation de listes d'indices actions et immos
        if (nrow(id_indices_action) < 1) {
            stop("[ESG : chargement_ESG] : Veuillez renseigner au moins un indice de type
                                              Action")
        }
        indice_action <- list()
        indice_action <- lapply(id_indices_action$num_index, function(y) {
            indice_action[[y]] <- read.csv2(paste(folder_Indices, id_indices_action[y, 2], sep = "/"))
        })
        names(indice_action) <- id_indices_action[, 1]

        if (nrow(id_indices_immo) < 1) {
            stop("[ESG : chargement_ESG] : Veuillez renseigner au moins un indice de type
                                            Immo")
        }
        indice_immo <- list()
        indice_immo <- lapply(id_indices_immo$num_index, function(y) {
            indice_immo[[y]] <- read.csv2(paste(folder_Indices, id_indices_immo[y, 2], sep = "/"))
        })
        names(indice_immo) <- id_indices_immo[, 1]

        # Chargement de l'indice inflation
        if (length(which(file_name[, 3] == "Inflation")) > 1) {
            stop("[ESG : chargement_ESG] : Veuillez renseigner
        ...
        if (length(which(file_name[, 3] == "yield_curve")) != 1) {
            stop("[ESG : chargement_ESG] : Veuillez renseigner
            ...
            if (ncol(temp_curve) != TERME) {
                ...
                if (!all(colnames(temp_curve) != paste("X", c(0.0833333, 0.25, 0.5, 0.75, 1:30, 40, 50)))) {
                    warning(paste("[ESG chargement : yield_curve] : Les colonnes des courbes de taux spot forward ",
                        ...
                        TERME, " (le nommage des colonnes doit etre 0.0833333, 0.25, 0.5, 0.75, 1, 2, ..., ", TERME, ").",
                        sep = ""
                    ))
                }
            }
        ...
        if (length(which(file_name[, 3] == "deflateur")) != 1) {
            stop("[ESG : chargement_ESG] : Veuillez renseigner exactement un lien de type deflateur")
        }
