...
##'
##' Une classe pour le stockage en memoire de variable de calcul au niveau du model point de retraite euro en phase de restitution
##' \code{\link{TabRetEuroRest}}.
##'
##' @name TabRetEuroRest
...
setClass(
    ...
    validity = function(object) {
        ...
        if (!is.numeric(object@tab[[1]])) {
            retval <- c(retval, "[TabRetEuroRest] : num_mp n'est pas entier\n")
        }
        if (!is.numeric(object@tab[[2]])) {
            retval <- c(retval, "[TabRetEuroRest] : prest n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[3]])) {
            retval <- c(retval, "[TabRetEuroRest] : pm_deb n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[4]])) {
            retval <- c(retval, "[TabRetEuroRest] : pm_fin n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[5]])) {
            retval <- c(retval, "[TabRetEuroRest] : bes_tx_cible n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[6]])) {
            retval <- c(retval, "[TabRetEuroRest] : nb_contr n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[7]])) {
            retval <- c(retval, "[TabRetEuroRest] : tx_cible n'est pas numeric\n")
        }
        if (!is.numeric(object@tab[[8]])) {
            retval <- c(retval, "[TabRetEuroRest] : pm_gar n'est pas numeric\n")
        }


        # Verification du nom des colonnes
        if (sum(names(object@tab) == c(
            "num_mp", "prest", "pm_deb", "pm_fin", "bes_tx_cible",
