...
setValidity(
    ...
    function(object) {
        ...
        if (dim(object@journal_achat_vente)[2] != nb_col) {
            retval <- c(
                retval,
                "[AlmEngine] : Nombre d'attributs incorrect, le journal des transactions ALMEngine est compose d'un DF de 7 colonnes \n"
            )
        }
        ...
        if (!identical(
            colnames(object@journal_achat_vente),
            c("annee", "operation", "type_actif", "num_mp", "num_index", "montant", "nb_unit", "pmvr")
        )) {
...
setMethod(
    ...
    definition = function(.Object, journal_achat_vente = data.frame()) {
        ...
        if (!missing(journal_achat_vente)) {
            if (ncol(journal_achat_vente) != nb_col | sum(
                names(journal_achat_vente) != c("annee", "operation", "type_actif", "num_mp", "num_index", "montant", "nb_unit", "pmvr")
            ) != 0L) {
