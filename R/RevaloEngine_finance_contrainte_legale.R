...
##' @param base_fin_etendu une valeur \code{numeric} comprenant la base totale de produits financiers
...
setMethod(
    ...
    signature = c(
        base_fin = "numeric", base_fin_etendu = "numeric", result_tech = "numeric", it_stock = "numeric",
    ...
    definition = function(base_fin, base_fin_etendu, result_tech, it_stock,
        ...
        if (length(base_fin_etendu) > 1L | length(result_tech) > 1L | length(dot_ppb) > 1L | length(marge_fin) > 1L) {
        ...
        solde_pb <- param_revalo@taux_pb_fi * base_fin_etendu + param_revalo@taux_pb_tech * result_tech * ind_result_tech +
        ...
        sum_base_fin <- sum(base_fin)
        if (sum_base_fin != 0) {
            rev_stock_nette_regl <- rev_stock_nette + add_rev_regl * base_fin / sum_base_fin
        } else { # Repartition au prorara si la base financiere est nulle
            rev_stock_nette_regl <- rev_stock_nette + add_rev_regl * 1 / length(base_fin)
        }


        # Output
        return(list(
            rev_stock_nette = rev_stock_nette_regl,
            marge_fin = marge_fin,
            ppb = ppb,
