...
setGeneric(name = "update_table_output_produit", def = function(table){standardGeneric("update_table_output_produit")})
setMethod(
    ...
    definition = function(table){
        ...
        table["frais"]      <- table["frais_fixe_prime"] + table["frais_var_prime"] + table["frais_fixe_prest"] + table["frais_var_prest"] + table["frais_fixe_enc"] + 
            table["frais_var_enc"] + table["frais_fin"]
